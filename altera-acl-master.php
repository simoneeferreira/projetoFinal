<?php
    ...
    $idACL = $_GET["idNivelAcesso"];
//Criando consulta para trazer o dados alterado
$queryACL = mysqli_query($conectaDB, "SELECT * FROM tb_nivelacesso WHERE id_nivelAcesso = $idACL") or die(mysqli_error($conectaDB));
while($aux = mysqli_fetch_assoc($queryACL)){
    $tipoAcesso = $aux["tipoAcesso"];
}
    ...
    <body>
        <div id="page">
            ...
            <main id="dashboard">
                ...
                <div>
                    <h2>Alterar nível de acesso - <?=$idACL?></h2>
                    <form action="update-acl-master.php" method="post" name="frm-cadastra">
                        <label>ACL</label>
                        <input type="text" name="nivel-acesso" value="<?=$tipoAcesso?>">
                        <input type="hidden" name="id-acl" value="<?=$idACL?>">
